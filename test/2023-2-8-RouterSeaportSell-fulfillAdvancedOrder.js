...
describe("seaport sell nft", function () {
    ...
    it("seaport sell erc721 nft", async () => {
        ...
        // await weth.approve(seaportAddress, BigNumber.from())
        ///////////////////// 2 encode date
        let SeaportLists = [];
        for (let i = 0; i < apiDatas.length; i++) {
            ...
            let wethApproveAmount = BigNumber.from("0")
            for (let i = 1; i < apiData.rawData.consideration.length; i++) {
                if(apiData.rawData.consideration[i].itemType == 2){
                    continue;
                }
                wethApproveAmount = wethApproveAmount.add(BigNumber.from(apiData.rawData.consideration[i].endAmount))
            }
            await weth.approve(sr.address, wethApproveAmount)
            ...
            const advancedOrder = {
                ...
                numerator: BigNumber.from('1'),  // number of buy
            ...
            // console.log("newNftOwner 111111:",newNftOwner)
            // let nftOwnerAmount = await weth.balanceOf(nftOwner)
            // console.log("nftOwnerAmount 111111:",nftOwnerAmount)
            ...
            // )
            // newNftOwner = await nft.ownerOf(nftId);
            // console.log("newNftOwner:",newNftOwner)
            // nftOwnerAmount = await weth.balanceOf(nftOwner)
            // console.log("nftOwnerAmount 111111:",nftOwnerAmount)
            // return
            ...
            // order type 0 fulfillBasicOrder and order type 1 fulfillAdvancedOrder
            const SeaportList = {
                inputDate: inputdata,
                ...
                tokenApproveAmount: wethApproveAmount
            };
        ...
        let input = ethers.utils.defaultAbiCoder.encode(
            [
                "(bytes inputDate, uint256 nftStandard, address nftAddress, uint256 nftTokenId, uint256 nftAmount,address tokenAddress,uint256 tokenApproveAmount)[]",
            ],
            [SeaportLists]
        );
        ...
        for (let i = 0; i < apiDatas.length; i++) {
            ...
            // console.log("nftOwnerAmount:",nftOwnerAmount)
